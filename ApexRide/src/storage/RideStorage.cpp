...
bool RideStorage::begin(IRideStore& store, const Config& config) {
    ...
    if (!store_->begin()) {
        APEX_LOGE("Storage failed to mount");
        return false;
    }

    if (!store_->ensureDirectory(config_.basePath)) {
        APEX_LOGE("Could not create %s", config_.basePath);
        return false;
    }
    ...
    APEX_LOGI("Storage: %u rides (%u unsynced), %llu of %llu KB free",
...
bool RideStorage::refresh() {
    ...
    if (!store_->list(config_.basePath, visitor)) {
        APEX_LOGE("Could not list %s", config_.basePath);
        return false;
    }

    if (visitor.overflowed()) {
        APEX_LOGW("More than %u rides on disk; only the first %u are indexed",
...
IRideFile* RideStorage::createRideFile(uint32_t rideId) {
    ...
    if (store_->exists(path)) {
        APEX_LOGE("Refusing to overwrite existing ride %s", path);
        return nullptr;
    }
...
bool RideStorage::writeSummary(const RideSummary& summary) {
    ...
    if (file == nullptr) {
        APEX_LOGE("Could not write %s", path);
        return false;
    }
    ...
    if (written != sizeof(copy) || !flushed) {
        APEX_LOGE("Short write on %s", path);
        return false;
    }
...
bool RideStorage::readSummary(uint32_t rideId, RideSummary& out) const {
    ...
    if (summary.rideId != rideId) {
        APEX_LOGW("Summary %s claims ride %u", path, static_cast<unsigned>(summary.rideId));
        return false;
    }
...
bool RideStorage::markSynced(uint32_t rideId) {
    ...
    if (entry == nullptr) {
        APEX_LOGW("markSynced: ride %u not found", static_cast<unsigned>(rideId));
        return false;
    }
    ...
    if (entry->summaryValid) {
        ...
        APEX_LOGE("markSynced: no usable summary for ride %u", static_cast<unsigned>(rideId));
        return false;
    }
    ...
    APEX_LOGI("Ride %u marked synced", static_cast<unsigned>(rideId));
    return true;
}
...
bool RideStorage::deleteRide(uint32_t rideId, bool force) {
    ...
    if (!synced && !force) {
        APEX_LOGW("Refusing to delete unsynced ride %u", static_cast<unsigned>(rideId));
        return false;
    }
    ...
    if (!dataRemoved) {
        APEX_LOGE("Could not delete %s", dataFile);
        return false;
    }

    APEX_LOGI("Deleted ride %u", static_cast<unsigned>(rideId));
    return refresh();
}
...
bool RideStorage::reclaimSpace(uint64_t neededBytes) {
    ...
    while (freeBytes() < neededBytes) {
        ...
        if (!foundVictim) {
            APEX_LOGW("Storage low (%llu KB free) and every ride is unsynced — keeping them all",
                    static_cast<unsigned long long>(freeBytes() / 1024));
            return false;
        }
...
bool RideStorage::rebuildSummary(uint32_t rideId, RideSummary& out) {
    ...
    if (!reader.readExact(&header, sizeof(header))) {
        APEX_LOGE("Ride %u: file too short to hold a header", static_cast<unsigned>(rideId));
        file->close();
        delete file;
        return false;
    }
    ...
    if (header.magic != kFileMagic || header.formatVersion != kFormatVersion ||
        ...
        APEX_LOGE("Ride %u: bad file header", static_cast<unsigned>(rideId));
        file->close();
        delete file;
        return false;
    }
    ...
    for (;;) {
        ...
        if (recordHeader.length > sizeof(payload)) {
            APEX_LOGW("Ride %u: record type %u claims %u bytes; stopping scan",
...
uint32_t RideStorage::recoverIncompleteRides() {
    ...
    for (size_t i = 0; i < rideCount_; ++i) {
        ...
        APEX_LOGW("Ride %u has no valid summary; rebuilding from data",
                static_cast<unsigned>(rideId));

        RideSummary rebuilt;
        if (!rebuildSummary(rideId, rebuilt)) {
            APEX_LOGE("Ride %u could not be recovered", static_cast<unsigned>(rideId));
            continue;
        }
        ...
        APEX_LOGI("Ride %u recovered: %u IMU / %u GNSS samples, %u s",
