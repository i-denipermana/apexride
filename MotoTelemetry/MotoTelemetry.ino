//
// MotoTelemetry V1 — offline-first motorcycle telemetry logger.
//
...
// MT_USE_MOCK_IMU / MT_USE_MOCK_GNSS in config.h swaps in the real hardware.
//
...
#if MT_USE_MOCK_IMU || MT_USE_MOCK_GNSS
...
#else
#error "Real ICM-20948 driver not implemented yet — keep MT_USE_MOCK_IMU set to 1"
#endif

#if MT_USE_MOCK_GNSS
#include "src/sensors/MockGnssSensor.h"
#else
#error "Real ATGM336H driver not implemented yet — keep MT_USE_MOCK_GNSS set to 1"
#endif
...
CalibrationStore  g_calibration(MT_NVS_NAMESPACE);
...
uint8_t  g_fallbackBuffer[MT_RECORD_BUFFER_BYTES];
...
TelemetrySystem::Config buildConfig() {
    ...
    config.orientation.kp                       = MT_MAHONY_KP;
    config.orientation.ki                       = MT_MAHONY_KI;
    config.orientation.useKinematicCorrection   = MT_USE_KINEMATIC_CORRECTION;
    config.orientation.minSpeedForCorrectionMps = MT_MIN_SPEED_FOR_CORRECTION_MPS;

    config.ride.startSpeedMps         = MT_RIDE_START_SPEED_MPS;
    config.ride.stopSpeedMps          = MT_RIDE_STOP_SPEED_MPS;
    config.ride.startHoldMs           = MT_RIDE_START_HOLD_MS;
    config.ride.waitingEnterMs        = MT_RIDE_WAITING_ENTER_MS;
    config.ride.waitingEnterNoGnssMs  = MT_RIDE_WAITING_ENTER_NO_GNSS_MS;
    config.ride.waitingTimeoutMs      = MT_RIDE_WAITING_TIMEOUT_MS;
    config.ride.sleepTimeoutMs        = MT_RIDE_SLEEP_TIMEOUT_MS;

    config.recorder.imuLogRateHz       = MT_IMU_LOG_RATE_HZ;
    config.recorder.blockSizeBytes     = MT_RECORD_BLOCK_BYTES;
    config.recorder.maxFlushIntervalMs = MT_MAX_FLUSH_INTERVAL_MS;
    config.recorder.summaryIntervalMs  = MT_SUMMARY_INTERVAL_MS;

    config.storage.basePath               = MT_RIDES_DIRECTORY;
    config.storage.minFreeBytesToStart    = MT_MIN_FREE_BYTES_TO_START;
    config.storage.minFreeBytesToContinue = MT_MIN_FREE_BYTES_TO_CONTINUE;

    config.fusionRateHz = MT_FUSION_RATE_HZ;
...
void printStorageBudget() {
    ...
    const float    bytesPerSecond =
        MT_IMU_LOG_RATE_HZ * (sizeof(RecordHeader) + sizeof(ImuRecord)) +
        MT_GNSS_RATE_HZ * (sizeof(RecordHeader) + sizeof(GnssRecord));
...
void printInfo() {
    Serial.printf("\nMotoTelemetry firmware 0x%04x, format v%u\n", kFirmwareVersion,
                  kFormatVersion);
...
void setup() {
    Serial.begin(MT_SERIAL_BAUD);
#if MT_WAIT_FOR_SERIAL
    while (!Serial) {
        delay(10);
    }
#else
    ...
    if (psramFound()) {
        uint8_t* psramBuffer = static_cast<uint8_t*>(ps_malloc(MT_RECORD_BUFFER_BYTES));
        if (psramBuffer != nullptr) {
            g_recordBuffer = psramBuffer;
            MT_LOGI("Telemetry buffer allocated in PSRAM (%u bytes)", MT_RECORD_BUFFER_BYTES);
        }
    } else {
        MT_LOGW("No PSRAM detected — using internal SRAM for the telemetry buffer");
    }

    // First boot has no filesystem yet; allow exactly one automatic format.
    if (!g_store.begin()) {
        MT_LOGW("Mount failed — formatting for first use");
        if (!g_store.format() || !g_store.begin()) {
            MT_LOGE("Filesystem unavailable; recording is disabled");
        }
    }
    ...
    MT_LOGW("Running on SIMULATED sensors — a scripted %u second ride starts at boot",
            static_cast<unsigned>(g_simulator.totalDurationMs() / 1000));
#endif

    MockImuSensor::Config imuConfig;
    imuConfig.sampleRateHz = MT_IMU_SAMPLE_RATE_HZ;
    g_imuSensor.setConfig(imuConfig);

    MockGnssSensor::Config gnssConfig;
    gnssConfig.updateRateHz = MT_GNSS_RATE_HZ;
    g_gnssSensor.setConfig(gnssConfig);

    if (!g_system.begin(buildConfig(), g_recordBuffer, MT_RECORD_BUFFER_BYTES)) {
        MT_LOGE("Telemetry system failed to start");
        return;
    }
    ...
    if (g_calibration.loadMountingOffset(storedMounting)) {
        ...
        MT_LOGW("No mounting calibration stored — park upright and level, then press 'c'");
    }
...
void loop() {
    ...
    if (nowMs - g_lastStatusMs >= MT_STATUS_INTERVAL_MS) {
