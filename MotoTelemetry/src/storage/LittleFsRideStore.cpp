...
bool LittleFsRideStore::begin() {
    ...
    if (!mounted_) {
        MT_LOGE("LittleFS mount failed on partition '%s'", config_.partitionLabel);
        return false;
    }

    MT_LOGI("LittleFS mounted: %llu KB total, %llu KB used",
...
bool LittleFsRideStore::format() {
    MT_LOGW("Formatting the LittleFS partition — all ride data will be lost");
    mounted_ = false;
