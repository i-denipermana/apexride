...
bool TelemetryRecorder::begin(const Config& config, uint8_t* buffer, size_t bufferSize) {
    ...
    if (buffer == nullptr || bufferSize < config.blockSizeBytes) {
        MT_LOGE("Recorder buffer too small: %u < %u", static_cast<unsigned>(bufferSize),
...
bool TelemetryRecorder::startRide(uint32_t startUnixTime, uint16_t calibrationVersion) {
    if (isRecording()) {
        MT_LOGW("startRide called while ride %u is open", static_cast<unsigned>(rideId_));
        return false;
    }
    ...
    if (!storage_.canStartRide()) {
        ...
        if (!storage_.reclaimSpace(storage_.minFreeBytesToStart())) {
            MT_LOGE("Not enough space to start a ride");
            return false;
        }
    }
    ...
    if (file_ == nullptr) {
        MT_LOGE("Could not create data file for ride %u", static_cast<unsigned>(rideId));
        return false;
    }
    ...
    if (!buffer_.append(&header, sizeof(header))) {
        MT_LOGE("Buffer cannot hold the file header");
        closeFile();
        return false;
    }
    ...
    MT_LOGI("Ride %u started (unix %u)", static_cast<unsigned>(rideId),
...
bool TelemetryRecorder::flush() {
    ...
    if (written != pending) {
        ...
        if (!storageFullLogged_) {
            MT_LOGE("Ride %u: wrote %u of %u bytes — storage is full",
...
bool TelemetryRecorder::endRide() {
    ...
    if (writeFailed_) {
        ...
        MT_LOGW("Ride %u had failed writes; rebuilding its summary from disk",
    ...
    MT_LOGI("Ride %u ended: %u s, %u.%02u km, %u IMU / %u GNSS samples, %u KB",
    ...
    if (droppedRecords_ > 0) {
        MT_LOGW("Ride %u dropped %u records", static_cast<unsigned>(rideId_),
